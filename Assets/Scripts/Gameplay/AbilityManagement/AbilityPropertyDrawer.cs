...
namespace Game.Gameplay.AbilityManagement
{
    ...
    public class AbilityPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Ability"), FocusType.Keyboard))
            {
                var menu = new GenericMenu();
                if (typeMap == null || typeMap.Count == 0)
                {
                    menu.AddDisabledItem(new GUIContent("No Abilities available"));
                    menu.ShowAsContext();
                    return;
                }
